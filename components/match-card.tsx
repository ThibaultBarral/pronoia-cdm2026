...
import { teamName, countryName } from "@/lib/i18n/teams";
...
export default function MatchCard({ match, index = 0 }: MatchCardProps) {
  ...
  const dateStr = date.toLocaleDateString(localeMeta[locale].htmlLang, {
    weekday: "short",
    day: "numeric",
    month: "short",
  });
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        >
          {t("matchCard.group", { group: match.group })}
        </Badge>
        <div className="flex items-center gap-1 text-[#888] text-xs">
          <MapPin size={11} />
          <span>{match.city}</span>
          <span className="ml-1 text-[10px] opacity-60">{countryName(match.country, locale)}</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 mb-3">
        ...
        <div className="flex flex-col items-center gap-1.5 flex-1 min-w-0">
          ...
          <span className="text-sm font-semibold text-center text-[#f0f0f0] leading-tight truncate w-full text-center">
            {teamName(match.homeTeam.name, locale)}
          </span>
        ...
        <div className="flex flex-col items-center gap-1 shrink-0">
          <div className="w-10 h-10 rounded-full border border-[#1f1f1f] bg-[#0a0a0a] flex items-center justify-center">
            <span className="text-xs font-bold text-[#888]">{t("matchCard.vs")}</span>
          </div>
        ...
        <div className="flex flex-col items-center gap-1.5 flex-1 min-w-0">
          ...
          <span className="text-sm font-semibold text-center text-[#f0f0f0] leading-tight truncate w-full text-center">
            {teamName(match.awayTeam.name, locale)}
          </span>
      ...
      {winamax && (
        <div className="flex items-center justify-around border-t border-[#1f1f1f] pt-3">
          ...
          <OddsChip label={t("matchCard.draw")} value={winamax.draw} />
          <div className="w-px h-8 bg-[#1f1f1f]" />
