...
import { ShieldPlus, AlertCircle, Check } from "lucide-react";
...
export default function AdminControls() {
  ...
  function grant() {
    ...
    startTransition(async () => {
      const res = await setAdminAction(email.trim(), true);
      if (res.ok) {
        setOk(`${email.trim()} est maintenant admin.`);
        setEmail("");
        router.refresh();
      } else {
  ...
  return (
    <div className="rounded-2xl glass p-4 mb-6">
      <div className="flex items-center gap-2 mb-3">
        <ShieldPlus size={16} className="text-[var(--accent)]" />
        <span className="text-sm font-bold text-[#f0f0f0]">Ajouter un administrateur</span>
      </div>
